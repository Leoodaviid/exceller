        ...
        <Container>
          <div className="max-w-3xl text-center mx-auto">
            ...
            <p className="mt-4 text-base text-muted-foreground text-balance">
              Clusters de conhecimento pensados para líderes de agências, times
              financeiros e squads de tecnologia que trabalham com a Exceller.
            </p>
          </div>
        </Container>
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
          {HELP_CATEGORIES.map((category, index) => (
            <Container
              key={category.title}
              delay={0.1 + index * 0.1}
              className="group relative flex h-full flex-col gap-4 rounded-3xl border border-border/60 bg-[#0D0D0D]/80 p-8 transition-all duration-300 hover:-translate-y-1 hover:border-primary/70 hover:bg-[#121212]/90"
            >
